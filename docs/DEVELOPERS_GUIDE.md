...
#### JWT Cookie Authentication Requires Proper Domain

**Problem:** When using JWT cookie authentication with RESTHeart's `authCookieSetter`, accessing the application via `localhost` causes login to fail silently.

**Root Cause:** RFC 6265 (HTTP cookie specification) defines special handling for `localhost` that causes inconsistent cookie behavior across browsers. Modern browsers often reject or ignore cookies set for the `localhost` domain, even though no error is shown.

**Symptoms:**
- Login form submits successfully
- User is redirected to protected page
- Immediately redirected back to login (infinite loop)
- No error messages in browser console
- No `rh_auth` cookie appears in browser DevTools

**Solution:** Use an RFC 6265 compliant domain mapped via `/etc/hosts`. The domain must match in all three places:

1. **Add to `/etc/hosts`** (on Windows: `C:\Windows\System32\drivers\etc\hosts`):
   ```
   127.0.0.1  local.getfacet.org
   ```

2. **Configure `authCookieSetter` in restheart.yml**:
   ```yaml
   /authCookieSetter:
     enabled: true
     name: rh_auth
     domain: getfacet.org  # Parent domain (no leading dot, RFC 6265 compliant)
     secure: false         # Must be false for HTTP (localhost development)
     http-only: true
     same-site: true
   ```

3. **Access via the mapped domain**:
   - ✅ **Works:** http://local.getfacet.org:8080
   - ❌ **Fails:** http://localhost:8080

**Important:** `getfacet.org` is just an example. You can use **any domain you prefer** (e.g., `myapp.local`, `facet.test`) as long as the domain in `/etc/hosts`, `authCookieSetter` config, and your browser URL all match.

**Alternative domains (all three places must match):**
...
**Option 2: `.test` domain**
```
# 1. /etc/hosts
127.0.0.1  facet.test

# 2. authCookieSetter
domain: test

# 3. Browser URL
http://facet.test:8080
```

**Rule:** The `domain` value should be the **parent domain** without subdomain:
- Browser URL: `local.getfacet.org` → Config domain: `getfacet.org`
- Browser URL: `myapp.local` → Config domain: `local`
- Browser URL: `facet.test` → Config domain: `test`

**Why this works:** Using a proper domain name (not `localhost`) ensures RFC 6265 compliant cookie handling. The `/etc/hosts` mapping routes the domain to `127.0.0.1` (same as `localhost`), but the browser treats it as a real domain with standard cookie behavior. The critical point is that **all three places must use the same domain** for the cookie to work.

**Production note:** In production with a real domain (e.g., `app.example.com`), set `domain: example.com` and `secure: true` to enable HTTPS-only cookies.
